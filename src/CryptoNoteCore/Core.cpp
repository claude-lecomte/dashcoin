...
std::error_code Core::validateTransaction(const CachedTransaction& cachedTransaction, TransactionValidatorState& state,
  ...
  auto error = validateSemantic(transaction, fee, blockIndex > parameters::KEY_IMAGE_CHECKING_BLOCK_INDEX);
...
std::error_code Core::validateSemantic(const Transaction& transaction, uint64_t& fee, bool checkKeyImages) {
  ...
  for (const auto& input : transaction.inputs) {
    ...
    if (input.type() == typeid(KeyInput)) {
      ...
      if (checkKeyImages && !(scalarmultKey(in.keyImage, L) == I)) {
